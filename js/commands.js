...
function getCommands(callback) {
    window.ccpAPI.getAnchorCompletions(null, function(data) {
        var suggestions = window.ccpUtil.map(data, function(item) {
            return { 'caption': 'Click anchor: ' + item.text, 'command': 'clickOnElement', 'args': item.selector, 'closeOnComplete' : true };
        });

        suggestions = getStaticCommands().concat(suggestions);
...
function clickOnElement(selector) {
    log.info('Trying to click on ' + selector);
    window.ccpAPI.selectAndClick(selector);
}